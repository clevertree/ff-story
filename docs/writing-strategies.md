...
- **Strict Fidelity to Source:** Every sentence in the `Draft` sections must have a direct parent in the `Chapter Beats` or `Synopsis`. Do not invent new causal links, technical requirements, theories, metaphors, or realizations that are not explicitly documented in the planning sections of the file or the world-building documents.
- **Fact-Checking:** Before finalizing a draft or plan, verify all technical details (like the 360-day Tun, the Aether-Drive, or lunar fragmentation) against `world-building/facts.md`.
